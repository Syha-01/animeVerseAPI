...
func (a *application) listAnimesHandler(w http.ResponseWriter, r *http.Request) {
	var input struct {
		Title        string
		Genres       []string
		data.Filters // CORRECTED: Embed with package prefix
	}
	...
	input.Filters.SortSafeList = []string{"id", "title", "score", "release_date", "-id", "-title", "-score", "-release_date"}

	// CORRECTED: Call validation function with package prefix
